    ...
    <>
      ...
      <AnimatePresence>
        {mobileMenuOpen && (
          ...
          >
            <Link to="/" onClick={() => { window.scrollTo(0,0); setMobileMenuOpen(false); }} className={`text-[2rem] font-semibold tracking-tight ${location.pathname === '/' ? 'text-black' : 'text-black/50'}`}>Home</Link>
            <Link to="/about" onClick={() => { window.scrollTo(0,0); setMobileMenuOpen(false); }} className={`text-[2rem] font-semibold tracking-tight ${location.pathname === '/about' ? 'text-black' : 'text-black/50'}`}>About</Link>
            <a 
              href="#contact" 
              onClick={() => setMobileMenuOpen(false)}
              className="mt-4 px-8 py-4 rounded-full bg-black hover:bg-[#ffb950] text-white hover:text-black transition-colors text-center text-[16px] font-medium"
            >
              Contact Us
            </a>
          </motion.div>
